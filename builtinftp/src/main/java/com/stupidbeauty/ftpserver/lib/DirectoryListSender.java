    ...
    else  // 是目录
    {
      ...
      else  // 列出成功
      {
        ...
        for (DocumentFile path : paths)  // 遍历每个文件
        {
          ...
          // 🔍 FIX: Check if nameOfFile is a path (starts with "/" or is empty) vs a specific filename
          // If it's a path, list all files. If it's a specific filename, only match that file.
